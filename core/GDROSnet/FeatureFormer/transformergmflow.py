...
def single_head_split_window_attention(q, k, v,
    ...
    attn = torch.softmax(scores, dim=-1) #tensor[8,1024,1024]=[b * num_splits * num_splits, 32*32,32*32]
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, source, target,
        ...
        """szx"""
        query, key, value = target, source, source
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attn2_ffn = TransformerLayer(d_model=d_model,
                                               nhead=nhead,
                                               attention_type=attention_type,
                                               ffn_dim_expansion=ffn_dim_expansion,
                                               with_shift=with_shift,
    ...
    def forward(self, source, target,
        ...
        source = self.cross_attn2_ffn(source, target,
                                     height=height,
                                     width=width,
                                     shifted_window_attn_mask=shifted_window_attn_mask,
                                     attn_num_splits=attn_num_splits,
