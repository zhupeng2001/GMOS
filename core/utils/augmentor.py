...
class FlowAugmentorlbl:
    ...
    def spatial_transform(self, img1, img2, lbl1, flow):
        # randomly sample scale
        ht, wd = img1.shape[:2]
        min_scale = np.maximum(
            (self.crop_size[0] + 8) / float(ht),
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            img1 = cv2.resize(img1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            lbl1 = cv2.resize(lbl1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            flow = cv2.resize(flow, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            flow = flow * [scale_x, scale_y]
        ...
        img1 = img1[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        img2 = img2[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        lbl1 = lbl1[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        flow = flow[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]

        return img1, img2, lbl1, flow

    def __call__(self, img1, img2, lbl1, flow):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        if self.pwc_aug:
            th, tw = self.crop_size
            ...
            img1, img2, lbl1, flow = self.spatial_transform(img1, img2, lbl1, flow)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        lbl1 = np.ascontiguousarray(lbl1)
        flow = np.ascontiguousarray(flow)

        return img1, img2, lbl1, flow


class SparseFlowAugmentorlbl:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False):
        # spatial augmentation params
        self.crop_size = crop_size
        self.min_scale = min_scale
    ...
    def spatial_transform(self, img1, img2, lbl1, flow, valid):
        pad_t = 0
        pad_b = 0
        pad_l = 0
        pad_r = 0
        ...
        if self.crop_size[1] > img1.shape[1]:
            pad_r = self.crop_size[1] - img1.shape[1]
        if pad_b != 0 or pad_r != 0:
            img1 = np.pad(img1, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant',
                          constant_values=((0, 0), (0, 0), (0, 0)))
            img2 = np.pad(img2, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant',
                          constant_values=((0, 0), (0, 0), (0, 0)))
            lbl1 = np.pad(lbl1, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant',
                          constant_values=((0, 0), (0, 0), (0, 0)))
            flow = np.pad(flow, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant',
                          constant_values=((0, 0), (0, 0), (0, 0)))
            valid = np.pad(valid, ((pad_t, pad_b), (pad_l, pad_r)), 'constant', constant_values=((0, 0), (0, 0)))
        # randomly sample scale
        ...
        flow = flow[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        valid = valid[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
        return img1, img2, lbl1, flow, valid

    def __call__(self, img1, img2, lbl1, flow, valid):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, lbl1, flow, valid = self.spatial_transform(img1, img2, lbl1, flow, valid)
